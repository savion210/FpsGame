...
public class LoginMenu : MonoBehaviour { 
	...
	void Update () {
		...
		if (isDatabaseSetup == true) {
			...
			if (part == 2) {
				// we are logged in - we have already transtioned into a new scene...
			}
			if (part == 3) {
				login_object.gameObject.SetActive (false);
				register_object.gameObject.SetActive (false);
	...
	IEnumerator sendLoginRequest (string username, string password) {
		...
		if (isDatabaseSetup == true) {
			...
			string response = e.Current as string; // << The returned string
			...
			if (response == "Success") {
				...
				UserAcountManager.instance.LogIn(username, password);
